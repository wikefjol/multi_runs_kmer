...
from src.utils.updates import send_discord_notification
# Training things
...
def extract_best_metrics(jsonl_path):
    """Extract the best metrics from a given JSONL log file."""
    ...
    with open(jsonl_path, 'r') as f:
        for line in f:
            ...
            val_accuracy = entry.get("val_accuracy") or entry.get("val_mlm_accuracy")
            if val_accuracy is not None and val_accuracy > best_accuracy:
    ...
    for config_file in os.listdir(CONFIG_DIR):
        if config_file.endswith(".json"):
            ...
            try:
                ...
                send_discord_notification(webhook_url, content= f"Starting {run_name}")
                ...
                try:
                    # Locate the logs
                    pretraining_log_path = os.path.join(run_dir, "logs", "pretraining.jsonl")
                    finetuning_log_path = os.path.join(run_dir, "logs", "finetuning.jsonl")

                    # Extract best metrics
                    pretrain_epoch, pretrain_metrics = extract_best_metrics(pretraining_log_path)
                    finetune_epoch, finetune_metrics = extract_best_metrics(finetuning_log_path)

                    # Construct the summary message
                    content = f"**Training Summary for {run_name}:**\n"
                    content += f"Config File: `{config_file}`\n\n"

                    if pretrain_metrics:
                        content += (
                            f"**Pretraining:**\n"
                            f" - Best Epoch: {pretrain_epoch}\n"
                            f" - Best Validation Accuracy: {pretrain_metrics['val_mlm_accuracy']:.4f}\n"
                            f" - Train Loss: {pretrain_metrics['train_avg_loss']:.4f}\n"
                            f" - Validation Loss: {pretrain_metrics['val_avg_loss']:.4f}\n\n"
                        )
                    else:
                        content += "**Pretraining:** No pretraining metrics found.\n\n"

                    if finetune_metrics:
                        content += (
                            f"**Finetuning:**\n"
                            f" - Best Epoch: {finetune_epoch}\n"
                            f" - Best Validation Accuracy: {finetune_metrics['val_accuracy']:.4f}\n"
                            f" - Train Loss: {finetune_metrics['train_loss']:.4f}\n"
                            f" - Validation Loss: {finetune_metrics['val_loss']:.4f}\n"
                        )
                    else:
                        content += "**Finetuning:** No finetuning metrics found.\n"

                    # Send the notification
                    send_discord_notification(webhook_url, content= content)

                except Exception as e:
                    print(f"Error sending Discord notification for config '{config_file}': {e}")
